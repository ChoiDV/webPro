...
<body>
	...
	<table>
		...
		<%/***********************************************************/
			...
			if(totalCnt==0){
				...
				ArrayList<BoardDto> dtos = dao.listBoard(startRow, endRow);/*startRow, endRow 추가*/
				for(BoardDto dto : dtos){
					...
					out.println("<a href='content.jsp?num="+ dto.getNum()+
																	"&pageNum=" + pageNum + "'>" 
																					+ dto.getSubject() +"</a></td>");
					//메일, 조회수
	...
	<%
		...
		for(int i=startPage ; i<=endPage ; i++){
			if(i==currentPage){ // 페이지 수만 출력(빨간 색 글씨로)
				out.println("[ <b>"+i+"</b> ] ");
		...
		if(endPage < pageCnt){ // 다음 블록이 있는지 여부 <a href='list.jsp?pageNum=다음블록첫페이지'>다음</a>
			out.println("[ <a href='list.jsp?pageNum="+ (endPage+1) +"'>다음블록</a> ] ");
		}
	%>
	</div>
</body>
